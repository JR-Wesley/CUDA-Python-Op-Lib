...
__global__ void vecAddKernelFloat4(float *A, float *B, float *C,
                                   const unsigned int N) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  int idx = i * 4;
...
void vecAddDevice(const float *A_h, const float *B_h, float *C_h, const int N,
                  const bool if_float4 = false) {
